...
try:
    ...
    for f_name in ["smoke_test.db-wal", "smoke_test.db-shm"]:
...
try:
    ...
    result = calc.true_margin(buy_price=100_000_000, sell_price=130_000_000, accounting_level=5, broker_relations_level=5)
    ...
    opps = [
        {"type_name": "PLEX", "buy_price": 3_900_000, "sell_price": 4_200_000, "daily_volume": 500, "avg_price": 4_050_000, "accounting_level": 5, "broker_relations_level": 5},
        {"type_name": "Tritanium", "buy_price": 4, "sell_price": 5, "daily_volume": 10_000_000, "avg_price": 4.5, "accounting_level": 3, "broker_relations_level": 3},
    ]
...
try:
    ...
    async def check_esi():
        async with aiohttp.ClientSession() as session:
            async with session.get("https://esi.evetech.net/latest/status/", timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status == 200:
...
try:
    ...
    if r.status_code == 200:
        ...
        print("  [WARN] Ollama returned unexpected status")
except Exception:
